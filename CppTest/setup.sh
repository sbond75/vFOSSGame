...
# If $1 is empty, set projectName to the default; otherwise, use the provided argument.
if [ -z "$1" ]; then
...
# https://github.com/urho3d/Urho3D/wiki#tutorials : see "Tutorials" header
# This script implements instructions described on https://github.com/urho3d/Urho3D/wiki/Setting-up-a-Project-(CMake)
#  - Documentation fix needed: actual folder is $URHO_ROOT/scripts, not just out in the open of the $URHO_ROOT.

ADD_IN "$URHO_ROOT/CMake" ./
...
# If the string starts with "UNKNOWN:" then we don't know the OS, so we exit.
if [[ "$machine" = UNKNOWN:* ]]; then
    echo "Unknown OS. Exiting."
    exit 1
fi

# Windows vs. others actions:
if [[ ("$machine" != "Cygwin" && "$machine" != "MinGw") || "$machine" == "Linux" || "$machine" != "Mac" ]]; then
    # Not Windows
    cp -a "$URHO_ROOT/script/cmake_generic.sh" ./script
else
    # Windows
    cp -a "$URHO_ROOT/script/cmake_generic.bat" ./script
fi


mkdir -p ./script
ADD_IN "../$URHO_ROOT/script/.bash_helpers.sh" ./script/

# Materials and resources
mkdir -p ./bin/
ADD_IN "../$URHO_ROOT/bin/CoreData" ./bin
ADD_IN "../$URHO_ROOT/bin/Data" ./bin
...
ADD_IN "../$URHO_ROOT/bin/Autoload" ./bin

# Prepare CMakeLists.txt macOS workaround if we're running macOS:
if [ "$machine" == "Mac" ]; then
    #additionalCMakeStuff='set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -std=c++11 -stdlib=libc++")'
    additionalCMakeStuff='
...
# Make main.cpp
# Code is from https://github.com/urho3d/Urho3D/wiki/First-Project
cat <<EOF >./main.cpp
#include <string>
#include <sstream>
